...
class JacketPage extends Common {
  constructor() {
    ...
    this.$jacketPrice = (price) =>
      $(`//span[contains(text(),"${price}")]/parent::a`);
    this.$jacketShopingBy = (price) => $(`//span[contains(text(),"${price}")]`);
    this.$jacketFiltered = (name) => $(`//a[contains(text(),"${name}")]`);
    this.$selectedJacket = () =>
      $("//span[contains(text(),'Lando Gym Jacket')]");
    this.$jacketSize = (size) =>
      $(
        `//span[contains(text(),'Size')]/following::div[contains(text(),"${size}")]`
      );
    this.$jacketColour = (colour) =>
      $(
        `//span[contains(text(),'Color')]/following::div[contains(@aria-label,"${colour}")]`
      );
