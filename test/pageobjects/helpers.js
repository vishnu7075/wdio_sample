...
module.exports = {
	 ...
	 * @returns {number}
	 */
	remainingTimeout: (timeout, execStarted) => timeout - (+new Date() - execStarted),
	...
	getUpcomingMonth(offset = 1) {
		...
		currentMonth < 13 ? (monthStart = currentMonth + '/1/' + currentYear) : (monthStart = (currentMonth % 12) + '/1/' + (currentYear + ~~(currentMonth / 12)));
		return monthStart;
	},
	...
	objectsPartiallyMatch(one, two) {
		...
		if (propertiesOfOne.length === 0 || propertiesOfTwo.length === 0) return false;
		...
		for (let i = 0; i < matchingProperties.length; i++) {
			if (one[matchingProperties[i]] !== two[matchingProperties[i]]) {
				console.log(`'${matchingProperties[i]}' property: ${one[matchingProperties[i]]} !== ${two[matchingProperties[i]]}`);
				result = false;
			}
		}
	...
	/**
	 * Generate 9 random digits where 0 can 't be at position 0, and no multiple 0 can go one after another
	 * @returns {number}
	 */
	generateSSN() {
		let {RandomSSN} = require('ssn');
		let randomSSN = new RandomSSN();
		return randomSSN.value().toString();
	},
